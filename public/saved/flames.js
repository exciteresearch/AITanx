pc.script.create('flames', function (context) {
    ...
    Flames.prototype = {
        initialize: function () {
            ...
            if (! matSpecial) {
                var flameSpecial = context.root.findByName('flame-special');
                matDefault = this.flame.model.material;
                matSpecial = flameSpecial.model.material;
                flameSpecial.destroy();
            }
        ...
        new: function(data) {
            ...
            console.log("flame", flame.targetPosition)
            flame.speed = data.sp * 50 * 0.5;
            
            // material and scale if special
            if (data.s) {
                flame.model.material = matSpecial;
                flame.setLocalScale(.3, .2, .3);
            } else {
                flame.model.material = matDefault;
                flame.setLocalScale(.2, .2, .2);
            }
